...
This repository holds the C++ source code for the cryptography and protocols, plus lightwallet functions.

A primary goal for this repository is its future convergence and reimplementation with an essential Bitcoin Confidential core library.


### Contents

* `contrib/quickbc-core-custom` is a Git submodule which contains a curated subset of the official Bitcoin Confidential C/C++ source code, eventually to be replaced by new official Bitcoin Confidential core repo

* `src` contains all the custom code written for this repo.
	* `serial_bridge_index` contains a central JSON interface to a set of central quickbc-core functions, documented below.
...
One example usage of this code is its transpilation into wasm and JS by quickbc-core-js.
...
We'll merge nearly anything constructive and relevant. There are lots of items slated for development or happening concurrently, so get in touch and we'll let you know if anyone's working on it.
...
* 🐮 `moneromooo-monero` Major contributor; Advisory

* 🦁 `ndorf` C++ expertise, architecture, code contributions
...
* The Bitcoin Confidential source, a slightly modified version of which is a dep of this project, can only be built on versions of iOS >= 9 due to required support for `thread_local` as used by `threadpool.cpp`.

* Not all Bitcoin Confidential `.cpp` files which are in `quickbc-core-custom` must be included for `quickbc-core-cpp` - such as when their symbols are not required by any code called by this project. See `CMakeLists.txt` for a list of files required for compilation.

* If you only want to call the C++ directly without using `serial_bridge_index` (described below), then that file-pair does not need to be included in your build. (See quickbc-app-ios link)
...
* AKA `create_address` in legacy quickbc-core-js
